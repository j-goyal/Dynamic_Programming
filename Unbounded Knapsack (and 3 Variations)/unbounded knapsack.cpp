// we can take multiple instances of weight
...
int knapSack(int w, int wt[], int val[], int n)  
{
	...
	if(wt[n-1]<=w)        // choice
	{
		return max(val[n-1] + knapSack(w-wt[n-1],wt,val,n), knapSack(w,wt,val,n-1));   // include(multiple times) or not include
	}
...
int knapSack_DP(int w, int wt[], int val[], int n)   
{
	...
	if(wt[n-1]<=w)        
	{
		dp[n][w] = max(val[n-1] + knapSack_DP(w-wt[n-1],wt,val,n), knapSack_DP(w,wt,val,n-1));  
	}
